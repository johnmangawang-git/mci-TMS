-- DEFINITIVE CENTRALIZED SCHEMA
-- Complete database schema that matches ALL JavaScript code requirements
-- This is the FINAL schema for centralized multi-user delivery tracking

-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Drop existing tables to ensure clean schema (CAUTION: This will delete data)
-- Comment out these lines if you want to preserve existing data
-- DROP TABLE IF EXISTS public.delivery_history CASCADE;
-- DROP TABLE IF EXISTS public.additional_cost_items CASCADE;
-- DROP TABLE IF EXISTS public.epod_records CASCADE;
...
-- Enhanced Deliveries table - MATCHES ALL JAVASCRIPT FIELD REFERENCES
CREATE TABLE IF NOT EXISTS public.deliveries (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    
    -- Core delivery fields (REQUIRED by JavaScript)
    ...
    -- Date/time fields (MULTIPLE FORMATS for compatibility)
    ...
    -- Additional delivery details (REQUIRED by Excel uploads)
    item_number TEXT,
    ...
    -- Additional data as JSON for flexibility (stores additionalCostItems, etc.)
    ...
    -- Signature data (for completed deliveries)
    signature_data TEXT,
    
    -- Completion timestamps (multiple formats for compatibility)
    completed_date DATE,
    completed_datetime TIMESTAMP WITH TIME ZONE,
    completed_timestamp TEXT
);

-- Enhanced Customers table - MATCHES ALL JAVASCRIPT FIELD REFERENCES
CREATE TABLE IF NOT EXISTS public.customers (
    ...
    email TEXT,
    phone TEXT,
    mobile_number TEXT, -- Additional mobile field
    address TEXT,
    vendor_number TEXT,
    
    -- Additional customer details (REQUIRED by JavaScript)
    contact_person TEXT,
    company_type TEXT,
    notes TEXT,
...
-- Additional Cost Items table (REQUIRED by analytics)
...
-- E-POD records table (REQUIRED by signature system)
...
-- Bookings/Calendar table (REQUIRED by calendar system)
...
-- CRITICAL: Remove unique constraint on dr_number to allow multiple deliveries with same DR
-- This is REQUIRED for Excel uploads with multiple items per DR
ALTER TABLE public.deliveries DROP CONSTRAINT IF EXISTS deliveries_dr_number_key;

-- Add composite unique constraint (dr_number + user_id + serial_number)
-- This prevents true duplicates while allowing multiple items per DR
ALTER TABLE public.deliveries 
ADD CONSTRAINT IF NOT EXISTS deliveries_unique_per_user 
UNIQUE (dr_number, user_id, serial_number);
...
CREATE POLICY IF NOT EXISTS "Users can view their own profile" ON public.user_profiles
...
CREATE POLICY IF NOT EXISTS "Users can view their own deliveries" ON public.deliveries
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own deliveries" ON public.deliveries
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own deliveries" ON public.deliveries
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete their own deliveries" ON public.deliveries
...
CREATE POLICY IF NOT EXISTS "Users can delete their own customers" ON public.customers
    FOR DELETE USING (auth.uid() = user_id);

-- Additional Cost Items
ALTER TABLE public.additional_cost_items ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "Users can view their own cost items" ON public.additional_cost_items
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own cost items" ON public.additional_cost_items
    FOR INSERT WITH CHECK (auth.uid() = user_id);

-- E-POD Records
ALTER TABLE public.epod_records ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "Users can view their own epod records" ON public.epod_records
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own epod records" ON public.epod_records
    FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY IF NOT EXISTS "Users can insert their own bookings" ON public.bookings
    FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY IF NOT EXISTS "Users can view their own sync queue" ON public.sync_queue
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own sync queue" ON public.sync_queue
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own sync queue" ON public.sync_queue
    FOR UPDATE USING (auth.uid() = user_id);

-- Activity Log
ALTER TABLE public.activity_log ENABLE ROW LEVEL SECURITY;

CREATE POLICY IF NOT EXISTS "Users can view their own activity" ON public.activity_log
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "System can insert activity logs" ON public.activity_log
...
DROP TRIGGER IF EXISTS deliveries_activity_log ON public.deliveries;
...
DO $$
BEGIN
    RAISE NOTICE '🎉 DEFINITIVE CENTRALIZED SCHEMA APPLIED SUCCESSFULLY!';
    RAISE NOTICE '✅ All JavaScript field references matched';
    RAISE NOTICE '✅ Multi-user support with RLS enabled';
    RAISE NOTICE '✅ Excel upload compatibility ensured';
    RAISE NOTICE '✅ Real-time subscriptions ready';
    RAISE NOTICE '✅ Signature system integrated';
    RAISE NOTICE '✅ Analytics data structure complete';
    RAISE NOTICE '🚀 System ready for centralized multi-user operation!';
END $$;